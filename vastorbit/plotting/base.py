...
from vastorbit.plotting.sql import PlottingBaseSQL
...
class PlottingBase(PlottingBaseSQL):
    ...
    def get_cmap(
        ...
        If  empty, vastorbit uses  the colors stored as
        a global variable.
        """
    ...
    def _map_method(
        ...
        if (
            ...
            if method in ["avg", "min", "max", "sum"]:
                ...
                aggregate = f"""APPROX_PERCENTILE({quote_ident(of)}, {q})"""
    ...
    def _compute_plot_params(
        ...
        # depending on the cardinality, the type, the VastColumn
        ...
        cast_b = "CAST(" if is_bool else ""
        cast_e = " AS DOUBLE)" if is_bool else ""
        ...
        # case when date
            ...
            query_result = _executeSQL(
                query=f"""
                    SELECT 
                        /*+LABEL('plotting._matplotlib._compute_plot_params')*/
                        FLOOR(1.00000000 * {converted_date} / {h}) * {h}, 
                        {aggregate} 
                    FROM {vdc._parent}
                    WHERE {vdc} IS NOT NULL 
                    GROUP BY 1 
        ...
        # case when numerical
            ...
            query_result = _executeSQL(
                query=f"""
                    SELECT
                        /*+LABEL('plotting._matplotlib._compute_plot_params')*/
                        FLOOR(1.00000000 * {cast_b}{vdc}{cast_e} / {h}) * {h},
                        {aggregate} 
                    FROM {vdc._parent}
                    WHERE {vdc} IS NOT NULL
                    GROUP BY 1
    ...
    def _compute_density(
        ...
        by: str, optional
            Column used to partition the data (categorical grouping).
        nbins: int, optional
        ...
        if len(columns) == 1 and not by:
            ...
            WITH bin_sequence AS (
                SELECT bin_id
                FROM UNNEST(SEQUENCE(1, {nbins})) AS t(bin_id)
            ),
            bins AS (
                SELECT 
                    WIDTH_BUCKET({column}, {min_val}, {max_val}, {nbins}) AS bin_id,
                    COUNT(*) AS count
                FROM {vdf.current_relation()}
            ...
            SELECT 
                {min_val} + (bs.bin_id - 1) * {bin_width} AS x,
                COALESCE(CAST(b.count AS DOUBLE), 0.0) / (t.total_count * {bin_width}) AS density
            FROM bin_sequence bs
            CROSS JOIN total t
            ...
            self.layout = {
                ...
                "x_label": self._clean_quotes(column),
                "y_label": "Density",
        ...
        # Single column with categorical grouping
            ...
            for cat in categories:
                ...
                filtered_data AS (
                    SELECT {column}
                    FROM {vdf.current_relation()}
                    WHERE {column} IS NOT NULL AND {filter_cond}
                ),
                bins AS (
                    SELECT 
                        WIDTH_BUCKET({column}, {min_val}, {max_val}, {nbins}) AS bin_id,
                ...
                SELECT 
                    ...
                    CASE 
                        ...
                        THEN COALESCE(CAST(b.count AS DOUBLE), 0.0) / (t.total_count * {bin_width})
                        ELSE 0.0
                    END AS density
                FROM bin_sequence bs
                CROSS JOIN total t
        ...
        # Multiple columns, no grouping
            ...
            self.layout = {
                ...
                "x_label": None,
                "y_label": "Density",
                "labels": np.array([self._clean_quotes(col) for col in columns]),
                "labels_title": None,
            }
    ...
    def _compute_statistics(
        ...
        if len(columns) == 1 and (by):
            expr = [
                f"MIN({columns[0]})",
                f"APPROX_PERCENTILE({columns[0]}, {q[0]})",
                f"APPROX_PERCENTILE({columns[0]}, 0.5)",
                f"APPROX_PERCENTILE({columns[0]}, {q[1]})",
                f"MAX({columns[0]})",
            ]
            if vdf[by].isnum() and not (vdf[by].isbool()):
                ...
                _by = ("CAST({} AS VARCHAR)", "varchar", "text")
                is_num_transf = False
            else:
    ...
    def _compute_pivot_table(
        ...
        for idx, column in enumerate(columns):
            ...
            cast_b = "CAST(" if vdf[column].isbool() else ""
            cast_e = " AS DOUBLE)" if vdf[column].isbool() else ""
            where = []
            if is_numeric:
                ...
                expr = f"""'[' 
                          || CAST(FLOOR(
                                 1.00000000 * {cast_b}{column}{cast_e}
                                 / {interval}) 
                                 * {interval} AS VARCHAR)
                          || ';' 
                          || CAST(FLOOR(
                ...
                if (interval > 1) or (vdf[column].category() == "float"):
                    ...
                    matrix += [
                        f"CAST(FLOOR({cast_b}{column}{cast_e}) AS VARCHAR) || ''"
                    ]
                order_by = f"""ORDER BY MIN(FLOOR(1.00000000 * {cast_b}{column}{cast_e} 
                ...
                matrix += [f"""FLOOR(1.00000000 * 
                              DATEDIFF('second',
                                '{min_date}',
                                {column})
                                / {interval}) * {interval}"""]
                is_column_date[idx] = True
                sql = f"""TIMESTAMPADD('second', CAST({columns[idx]} AS INT), 
                                                 CAST('{min_date}' AS TIMESTAMP))"""
        ...
        over = " * 1.00000000 / " + str(vdf.shape()[0]) if (method == "density") else ""
        if len(columns) == 1:
            cast = to_varchar(vdf[columns[0]].category(), matrix[-1])
            res = TableSample.read_sql(query=f"""
                    SELECT 
                        {cast},
                        {aggregate}{over} 
                    FROM {vdf}
                    {where}
                    GROUP BY 1 {order_by}""").to_numpy()
    ...
    def _compute_contour_grid(
        ...
        self.layout = {
            ...
            "func": func,
            "func_repr": (
                func_name if not isinstance(func_name, NoneType) else func_repr
            ),
        }
    ...
    def _compute_aggregate(
        ...
        self._init_check(dim=len(columns), is_standard=is_standard)
        if method == "density":
            over = " * 1.00000000 / " + str(float(vdf.shape()[0]))
        else:
            over = ""
        if isinstance(method, NoneType) and of:
    ...
    def _filter_line(
        ...
        if limit_over > 0:
            X = create_new_vdf(f"""
                SELECT * FROM (
                    SELECT *, 
                           ROW_NUMBER() OVER (PARTITION BY {order_by} 
                                              ORDER BY {columns[1]} DESC) as rn
                    FROM {X}
                ) x
                WHERE rn <= {limit_over}""").sort(columns=[order_by, columns[1]])
        if limit > 0:
            X = X[:limit]
        X = X.to_numpy()
    ...
    def _compute_range(
        ...
        for column in columns:
            expr += [
                f"APPROX_PERCENTILE({column}, {q[0]})",
                f"APPROX_PERCENTILE({column}, 0.5)",
                f"APPROX_PERCENTILE({column}, {q[1]})",
            ]
        X = (
            vdf.between(
                column=order_by, start=order_by_start, end=order_by_end, inplace=False
    ...
    def _filter_line_animated_scatter(
        ...
        if catcol:
            cols += [catcol]
        ctype = vdf[order_by].ctype().upper()
        if ctype == "FLOAT":
            ctype = "DOUBLE"
        where = (
            f" AND {order_by} > CAST('{order_by_start}' AS {ctype})"
            if (order_by_start)
            else ""
        )
        where += (
            f" AND {order_by} < CAST('{order_by_end}' AS {ctype})"
            if (order_by_end)
            else ""
        )
        query_result = _executeSQL(
            query=f"""
                ...
                FROM 
                    (SELECT 
                        ...
                        {by_},
                        ROW_NUMBER() OVER (PARTITION BY {order_by} 
                                           ORDER BY {order_by}, {cols[2]} DESC) as rn
