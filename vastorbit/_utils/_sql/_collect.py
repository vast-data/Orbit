...
def _dict_to_json_string(
    ...
    .. ipython:: python
        ...
        # Import the function.
        from vastorbit._utils._sql._collect import _dict_to_json_string
    ...
    json = "{"
    if name:
        json += f'"vastorbit_fname": "{name}", '
    if path:
        json += f'"vastorbit_fpath": "{path}", '
    if add_identifier:
        json += f'"vastorbit_id": "{gb_conn.vo_session_identifier}", '
...
def save_to_query_profile(
    ...
    path: Optional[str] = None,
    json_dict: Optional[dict] = None,
    query_label: str = "vastorbit_json",
    ...
    :py:func:`~vastorbit._utils._sql._collect.save_vastorbit_logs`
    """
...
def save_vastorbit_logs(func: Callable) -> Callable:
    """
    save_vastorbit_logs decorator. It
    ...
    .. note::
        ...
        function to the VAST meta tables,
        making it possible to analyze vastorbit
        usage.
    """
    ...
    def func_prec_save_logs(*args, **kwargs) -> Any:
        name = func.__name__
        path = func.__module__.replace("vastorbit.", "")
        json_dict = {}
