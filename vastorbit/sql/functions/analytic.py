...
def conditional_change_event(expr: SQLExpression) -> StringSQL:
    ...
    .. code-block:: python
        ...
        df["cchange_event"] = vof.conditional_change_event(df["y"] > 0)._over(order_by = [df["x"]])
        display(df)
...
def conditional_true_event(expr: SQLExpression) -> StringSQL:
    ...
    .. code-block:: python
        ...
        df["ctrue_event"] = vof.conditional_true_event(df["y"] > 0)._over(order_by = [df["x"]])
        display(df)
