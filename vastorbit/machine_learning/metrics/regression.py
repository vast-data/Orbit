...
from vastorbit._typing import NoneType, PythonNumber, SQLRelation
...
FUNCTIONS_REGRESSION_SQL_DICTIONARY = {
    ...
    "max_error": "CAST(MAX(ABS({y_true} - {y_score})) AS DOUBLE)",
    "median": "APPROX_PERCENTILE(ABS({y_true} - {y_score}), 0.5)",
    "median_absolute_error": "APPROX_PERCENTILE(ABS({y_true} - {y_score}), 0.5)",
    ...
    "rsquared": "1.00000000 - (SUM(POWER({y_true} - {y_score}, 2))) / (SUM(POWER({y_true} - _vastorbit_avg_y_true, 2)))",
    "r2_adj": "1.00000000 - ((1.00000000 - (1.00000000 - (SUM(POWER({y_true} - {y_score}, 2))) / (SUM(POWER({y_true} - _vastorbit_avg_y_true, 2))))) * (MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END) - 1) / (MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END) - {k} - 1))",
    "rsquared_adj": "1.00000000 - ((1.00000000 - (1.00000000 - (SUM(POWER({y_true} - {y_score}, 2))) / (SUM(POWER({y_true} - _vastorbit_avg_y_true, 2))))) * (MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END) - 1) / (MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END) - {k} - 1))",
    "r2adj": "1.00000000 - ((1.00000000 - (1.00000000 - (SUM(POWER({y_true} - {y_score}, 2))) / (SUM(POWER({y_true} - _vastorbit_avg_y_true, 2))))) * (MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END) - 1) / (MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END) - {k} - 1))",
    "r2adjusted": "1.00000000 - ((1.00000000 - (1.00000000 - (SUM(POWER({y_true} - {y_score}, 2))) / (SUM(POWER({y_true} - _vastorbit_avg_y_true, 2))))) * (MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END) - 1) / (MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END) - {k} - 1))",
    "aic": "MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END) * LN(MAX(_vastorbit_mse)) + 2 * ({k} + 1) + (POWER(2 * ({k} + 1), 2) + 2 * ({k} + 1)) / (MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END) - {k} - 2)",
    "bic": "MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END) * LN(MAX(_vastorbit_mse)) + ({k} + 1) * LN(MAX(CASE WHEN _vastorbit_cnt_y_true > _vastorbit_cnt_y_score THEN _vastorbit_cnt_y_score ELSE _vastorbit_cnt_y_true END))",
}
...
def bic_score(
    ...
    .. ipython:: python
        ...
        from vastorbit.machine_learning.metrics import bic_score
...
def explained_variance(y_true: str, y_score: str, input_relation: SQLRelation) -> float:
    ...
    .. ipython:: python
        ...
        from vastorbit.machine_learning.metrics import explained_variance
...
def max_error(y_true: str, y_score: str, input_relation: SQLRelation) -> float:
    ...
    .. ipython:: python
        ...
        from vastorbit.machine_learning.metrics import max_error
...
def mean_squared_error(
    ...
    .. ipython:: python
        ...
        from vastorbit.machine_learning.metrics import mean_squared_error
...
def mean_squared_log_error(
    ...
    .. ipython:: python
        ...
        from vastorbit.machine_learning.metrics import mean_squared_log_error
...
def median_absolute_error(
    ...
    .. ipython:: python
        ...
        from vastorbit.machine_learning.metrics import median_absolute_error
...
def quantile_error(
    ...
    .. ipython:: python
        ...
        from vastorbit.machine_learning.metrics import quantile_error
...
def r2_score(
    ...
    .. ipython:: python
        ...
        from vastorbit.machine_learning.metrics import r2_score
...
def anova_table(
    ...
    .. ipython:: python
        ...
        from vastorbit.machine_learning.metrics import anova_table
...
def regression_report(
    ...
    for m in selected_metrics:
        ...
        if m in ("aic", "bic") and not mse_in:
            ...
            q_subquery += [f"{mse} AS _vastorbit_mse"]
            mse_in = True
        if m in ("r2", "r2_adj") and not avg_in:
            q_subquery += [
                f"AVG(CASE WHEN {y_true} IS NULL OR _y_score IS NULL THEN NULL ELSE {y_true} END) OVER() AS _vastorbit_avg_y_true"
            ]
            avg_in = True
    if len(q_subquery) > 0:
        relation = f"""
