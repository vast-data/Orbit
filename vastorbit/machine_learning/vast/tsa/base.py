...
class TimeSeriesModelBase(VASTModel):
    ...
    def _ismultivar(self) -> bool:
        ...
        """
        try:
            return self.get_VAST_attributes("num_predictors")["num_predictors"][0] > 1
        except:
            return False
    ...
    def _compute_attributes(self) -> None:
        ...
        if self._ismultivar():
            p = self.parameters["p"]
            self.intercept_ = np.array(self.get_VAST_attributes("mean")["value"])
            minmax = (
                VastFrame(self.input_relation)[self.y]
                .aggregate(["min", "max"])
                .to_numpy()
            )
            self.min_ = minmax[:, 0]
            ...
            for i in range(1, p + 1):
                phi = np.array(self.get_VAST_attributes(f"phi_(t-{i})").to_numpy())
                self.phi_ += [phi[:, 1:].astype(float)]
            self.phi_ = np.array(self.phi_)
        else:
            coefficients = self.get_VAST_attributes("coefficients")
            i = 1
            if "p" in self.parameters:
                p = self.parameters["p"]
                self.intercept_ = coefficients["value"][0]
            else:
                mean_tmp = self.get_VAST_attributes("mean")
        ...
        try:
            self.mse_ = self.get_VAST_attributes("mean_squared_error")[
                "mean_squared_error"
            ][0]
        except:
            self.mse_ = None
        self.n_ = self.get_VAST_attributes("accepted_row_count")["accepted_row_count"][
            0
        ]
    ...
    def fit(
        ...
        if len(y) > 1:
            ...
            y_str = self.y
        tmp_view = False
        if isinstance(input_relation, VastFrame) and self._is_native:
            tmp_view = True
        ...
        # Fitting
        if self._is_native:
            parameters = self._get_VAST_param_dict()
    ...
    def _compute_features_importance(self) -> None:
        ...
        if self._model_type == "MA" or (
            ...
            self.feature_importances_ = 100.0 * self.phi_ / sum(abs(self.phi_))
    ...
    def predict(
        ...
        if not (isinstance(vdf, NoneType)):
            ...
            if not (isinstance(start, NoneType)) and str(method).lower() == "forecast":
                relation = (
                    f"(SELECT * FROM {vdf} ORDER BY {ts} LIMIT {start}) VASTORBIT_SUBTABLE"
                )
            sql += f" FROM {relation}"
            no_relation = False
        ...
        if output_estimated_ts:
            ...
            delta = f"""
                (SELECT 
                 ...
                 FROM (SELECT 
                        {delta_ts} 
                       FROM {self.input_relation}) VASTORBIT_SUBTABLE)"""
            if freq in ("m", "months"):
            ...
            sql = f"""
                SELECT
                    {estimation},
                    {prediction}{stde_out}
                FROM ({sql}) VASTORBIT_SUBTABLE{where}"""
    ...
    def _evaluation_relation(
        ...
        sql = f"""
            ...
            FROM 
                ...
                NATURAL JOIN
                (SELECT * FROM {prediction}) AS prediction_relation) VASTORBIT_SUBTABLE"""
        return clean_query(sql)
