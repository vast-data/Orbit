...
from vastorbit.machine_learning.model_selection.hp_tuning.cv import grid_search_cv
from vastorbit.machine_learning.model_selection.hp_tuning.param_gen import (
    gen_params_grid,
)
from vastorbit.machine_learning.model_selection.variables_selection import stepwise
...
from vastorbit.machine_learning.vast.cluster import NearestCentroid
...
class AutoML(VASTModel):
    ...
    estimator: list / 'native' / 'all' / 'fast' / object
        List  of VAST  estimators with a fit  method.
        Alternatively,  you can specify 'native' for all
        native  VAST models, 'all' for all  vastorbit
    ...
    def fit(
        ...
        if isinstance(X, NoneType):
            ...
            if not isinstance(input_relation, VastFrame):
                X = VastFrame(input_relation).get_columns(
        ...
        if isinstance(self.parameters["estimator"], str):
            ...
            for elem in self.parameters["estimator"]:
                assert isinstance(
                    ...
                    f"estimator must be a list of vastorbit estimators. Found {elem}."
                )
        ...
        if not isinstance(self.preprocess_, NoneType):
            ...
            self.preprocess_.final_relation_ = VastFrame(self.preprocess_.sql_)
    ...
    def features_importance(
        ...
        if self.stepwise_:
            ...
            vo_plt.ImportanceBarChart(data=data, layout=layout).draw(**kwargs)
    ...
    def plot(
        ...
        **style_kwargs,
        ...
        if mltype == "champion":
            ...
            return vo_plt.ChampionChallengerPlot(data=data, layout=layout).draw(
                **kwargs
            )
        else:
            vo_plt, kwargs = self.get_plotting_lib(
            ...
            return vo_plt.StepwisePlot(data=data, layout=layout).draw(**kwargs)
