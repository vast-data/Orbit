...
from vastorbit.machine_learning.vast.decomposition import PCA


class AutoDataPrep(VASTModel):
    """
    ...
    na_method: str, optional
        Method for handling missing values.
            auto: Mean  for the numerical features  and
                  ...
                  categorical  VastColumns.  For  time
    ...
    def fit(
        ...
        ts: str, optional
            Time series  VastColumn used to order the
            data.
            The VastColumn type must be date-like (date,
            datetime, timestamp...).
        ...
        if ts:
            if self.parameters["rule"] == "auto":
                ...
                if by:
                    ...
                    by_tmp = ""
                vdf_tmp["vastorbit_time_delta"] = f"""
                    ({ts}::timestamp 
                  - (LAG({ts}) OVER ({by_tmp}ORDER BY {ts}))::timestamp) 
                  / '00:00:01'"""
                vdf_tmp = vdf_tmp.groupby(["vastorbit_time_delta"], ["COUNT(*) AS cnt"])
                rule = _executeSQL(
                    query=f"""
                        SELECT 
                            /*+LABEL('learn.delphi.AutoDataPrep.fit')*/
                            vastorbit_time_delta 
                        FROM {vdf_tmp} 
                        ORDER BY cnt DESC 
                        LIMIT 1""",
