...
from vastorbit.machine_learning.metrics import aic_score, bic_score
...
def stepwise(
    ...
    **style_kwargs,
    ...
    ----------
    estimator: VASTModel
        VAST estimator with a fit method. It must be a
    ...
    **style_kwargs
    ...
    if show:
        ...
        res.step_wise_ = vo_plt.StepwisePlot(data=data, layout=layout).draw(**kwargs)
        data = {
            "importance": np.array(importance),
        }
        layout = {"columns": copy.deepcopy(res["variable"])}
        res.importance_ = vo_plt.ImportanceBarChart(data=data, layout=layout).draw(
            **kwargs
        )
    return res
