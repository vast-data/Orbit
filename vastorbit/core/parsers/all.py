...
from vastorbit.core.parsers._utils import extract_col_dt_from_query, extract_compression
...
def read_file(
    ...
    JSON, or Avro format.
    This function uses the VAST complex data type.
    ...
    .. ipython:: python
        ...
        drop(name = "default.titanic_subset")
        drop(name = "default.titanic_sub_dtypes")
    ...
    .. note::
        ...
        The :py:func:`~vastorbit.all.read_file`
    ...
    result = _executeSQL(
        query=f"""
            SELECT INFER_TABLE_DDL ('{path}' 
                                    USING PARAMETERS
                                    format='{file_format}',
                                    table_name='y_vastorbit',
                                    table_schema='x_vastorbit',
                                    table_type='native',
    ...
    result = result.replace(
        'create table "x_vastorbit"."y_vastorbit"', create_statement
    )
    if ";\n copy" in result:
        result = result.split(";\n copy")
        if temporary_local_table:
    ...
    if len(result) == 1:
        ...
        try:
            ...
            raise
    return VastFrame(input_relation=table_name, schema=schema)
