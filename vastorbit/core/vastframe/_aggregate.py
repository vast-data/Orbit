...
from vastorbit.core.vastframe._eval import vDFEval, vDCEval
...
class vDFAgg(vDFEval):
    ...
    def aggregate(
        ...
        columns: SQLColumns, optional
            List of  the VastColumn's names. If empty,  depending on the
            aggregations, all or only numerical VastColumns are used.
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_aggregate_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_aggregate_table.html

        .. note:: All the calculations are pushed to the database.

        .. hint::

            When the VastFrame includes a large number of columns and many aggregates
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.aggregate` :
              Aggregations for a specific column.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.describe` :
              Summarizes the information within the column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.describe` :
        ...
        for fun in func:
            ...
            elif fun.lower() in ("approx_unique", "approx_distinct"):
                agg_approx += [fun.lower()]
        ...
        for idx, column in enumerate(columns):
            for fun in func:
                cast_b = "CAST(" if (self[column].isbool()) else ""
                cast_e = " AS INT)" if (self[column].isbool()) else ""
                if fun.lower().startswith(("bool_and", "bool_or")):
                    cast_b, cast_e = "CAST(", " AS BOOLEAN)"
                ...
                if pre_comp != "VASTORBIT_NOT_PRECOMPUTED":
                    ...
                    expr = f"KURTOSIS({cast_b}{column}{cast_e})"

                elif fun.lower() in ("skewness", "skew"):
                    expr = f"SKEWNESS({cast_b}{column}{cast_e})"

                elif fun.lower() in ("jb"):
                    expr = f"1.00000000 * COUNT({column}) / 6 * (POWER(SKEWNESS({cast_b}{column}{cast_e}), 2) + 0.25 * POWER(KURTOSIS({cast_b}{column}{cast_e}), 2))"
                    ...
                    expr = (
                        f"MAX({cast_b}{column}{cast_e}) - MIN({cast_b}{column}{cast_e})"
                    )
                    ...
                    expr = f"APPROX_DISTINCT({column})"

                elif fun.lower() == "count":
                    expr = f"COUNT({column})"

                elif fun.lower() in ("approx_median", "approximate_median"):
                    expr = f"APPROX_PERCENTILE({cast_b}{column}{cast_e}, 0.5)"

                elif fun.lower() == "median":
                    expr = f"APPROX_PERCENTILE({cast_b}{column}{cast_e}, 0.5)"

                elif fun.lower() in ("std", "stddev", "stdev"):
                    expr = f"STDDEV({cast_b}{column}{cast_e})"

                elif fun.lower() in ("var", "variance"):
                    expr = f"VARIANCE({cast_b}{column}{cast_e})"

                elif fun.lower() in ("mean", "avg"):
                    expr = f"AVG({cast_b}{column}{cast_e})"

                elif fun.lower() == "iqr":
                    expr = f"""
                        APPROX_PERCENTILE({cast_b}{column}{cast_e}, 0.75) 
                      - APPROX_PERCENTILE({cast_b}{column}{cast_e}, 0.25)"""
                    ...
                    expr = f"""AVG(
                                CASE 
                                    WHEN {cast_b}{column}{cast_e} >= {q95} 
                                        THEN {cast_b}{column}{cast_e} 
                                    ELSE NULL 
                                END)"""

                elif fun.lower() == "sem":
                    expr = f"STDDEV({cast_b}{column}{cast_e}) / SQRT(COUNT({column}))"

                elif fun.lower() == "aad":
                    mean = self[column].avg()
                    expr = f"1.00000000 * SUM(ABS({cast_b}{column}{cast_e} - {mean})) / COUNT({column})"

                elif fun.lower() == "mad":
                    median = self[column].median()
                    expr = f"APPROX_PERCENTILE(ABS({cast_b}{column}{cast_e} - {median}), 0.5)"
                    ...
                    if self.shape()[0] == 0:
                        ...
                        expr = f"CAST(ROUND(100.0000000 * COUNT({column}) / {self.shape()[0]}, 3) AS DOUBLE)"

                elif "{}" not in fun:
                    expr = f"{fun.upper()}({cast_b}{column}{cast_e})"
        ...
        try:
            ...
            try:
                result = [
                    item if isinstance(item, (bool)) else float(item) for item in result
                ]
    ...
    def describe(
        ...
        """
        This function aggregates the VastFrame using multiple statistical
        ...
        applied depend on the data types of the VastColumns. For example,
        ...
        .. note::
            ...
            ``VastFrame.``:py:meth:`~vastorbit.VastFrame.aggregate` method, as it
        ...
        method: str, optional
            The describe method.
             ...
             - all:
                Aggregates all statistics for all VastColumns.
                The exact method depends on the VastColumn type
                (numerical  dtype:  numerical; timestamp  dtype:
                range; categorical dtype: length)
             - auto:
                Sets the method  to  ``numerical`` if  at least
                one VastColumn of the VastFrame is numerical,
                ``categorical`` otherwise.
             - categorical:
                Uses only categorical aggregations.
             - length:
                Aggregates the VastFrame using numerical
             ...
             - numerical:
                ...
                method.
             - range:
                Aggregates  the  VastFrame   using  multiple
        ...
        columns: SQLColumns, optional
            List of the VastColumns names.  If empty, the  VastColumns are
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_describe_num_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_describe_num_table.html
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_describe_cat_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_describe_cat_table.html
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_describe_all_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_describe_all_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.aggregate` : Aggregations for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.aggregate` : Aggregations for specific columns.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.describe` :
              Summarizes the information within the column.
        """
        if method == "auto":
        ...
        if method == "numerical":
            if not columns:
                ...
                for column in columns:
                    assert self[column].isnum(), TypeError(
                        f"VastColumn {column} must be numerical to run describe"
            ...
            func = [
                ...
                "percent",
                "count",
                "SUM(CASE WHEN LENGTH(CAST({} AS VARCHAR)) = 0 THEN 1 ELSE 0 END) AS empty",
                "AVG(LENGTH(CAST({} AS VARCHAR))) AS avg_length",
                "STDDEV(LENGTH(CAST({} AS VARCHAR))) AS stddev_length",
                "CAST(MIN(LENGTH(CAST({} AS VARCHAR))) AS INT) AS min_length",
                """CAST(APPROX_PERCENTILE(LENGTH(CAST({} AS VARCHAR)), 0.25) AS INT) AS "25%_length" """,
                """CAST(APPROX_PERCENTILE(LENGTH(CAST({} AS VARCHAR)), 0.5) AS INT) AS "50%_length" """,
                """CAST(APPROX_PERCENTILE(LENGTH(CAST({} AS VARCHAR)), 0.75) AS INT) AS "75%_length" """,
                "CAST(MAX(LENGTH(CAST({} AS VARCHAR))) AS INT) AS max_length",
            ]
            values = self.aggregate(
                func=func,
                columns=columns,
            ...
            if catcol:
                tmp = self.aggregate(
                    func=[
                        ...
                        "top",
                        "top_percent",
                        "AVG(LENGTH(CAST({} AS VARCHAR))) AS avg",
                        "STDDEV(LENGTH(CAST({} AS VARCHAR))) AS stddev",
                        "CAST(MIN(LENGTH(CAST({} AS VARCHAR))) AS INT) AS min",
                        """CAST(APPROX_PERCENTILE(LENGTH(CAST({} AS VARCHAR)), 0.25) AS INT) AS "approx_25%" """,
                        """CAST(APPROX_PERCENTILE(LENGTH(CAST({} AS VARCHAR)), 0.5) AS INT)  AS "approx_50%" """,
                        """CAST(APPROX_PERCENTILE(LENGTH(CAST({} AS VARCHAR)), 0.75) AS INT) AS "approx_75%" """,
                        "CAST(MAX(LENGTH(CAST({} AS VARCHAR))) AS INT) AS max",
                        "CAST(MAX(LENGTH(CAST({} AS VARCHAR))) AS INT) - CAST(MIN(LENGTH(CAST({} AS VARCHAR))) AS INT) AS range",
                        "SUM(CASE WHEN LENGTH(CAST({} AS VARCHAR)) = 0 THEN 1 ELSE 0 END) AS empty",
                    ],
                    columns=catcol,
                    ncols_block=ncols_block,
    ...
    def groupby(
        ...
        """
        This method facilitates the aggregation of the VastFrame by
        ...
        ----------
        columns: SQLColumns
            List  of the  VastColumns  used  to group the elements  or a
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_groupby_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_groupby_table.html

        Alternatively, you can achieve grouping using vastorbit SQL
        ...
        .. code-block:: python
            ...
            data.groupby(
                columns = ["x"],
                expr = [
                    vof.avg(data["y"])._as("avg_y"),
                    vof.min(data["z"])._as("min_z"),
                ],
            )
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_groupby_table_2.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_groupby_table_2.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.aggregate` : Aggregations for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.aggregate` : Aggregates for particular columns.
        """
    ...
    def aad(
        ...
        **agg_kwargs,
        ...
        .. warning::
            ...
            To compute aad, vastorbit needs to execute multiple
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_aad_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_aad_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.aad` :
              Average Absolute Deviation for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.std` :
    ...
    def all(
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_all_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_all_table.html
        ...
        .. seealso::
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.any` : Boolean OR Aggregation.
        """
    ...
    def any(
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_any_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_any_table.html
        ...
        .. seealso::
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.all` : Boolean AND Aggregation.
        """
    ...
    def avg(
        ...
        **agg_kwargs,
        ...
        """
        This operation aggregates the VastFrame using the ``AVG``
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_avg_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_avg_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.avg` : Aggregations for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.max` : Maximum for particular columns.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.min` : Minimum for particular columns.
        """
    ...
    def count(
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_count_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_count_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.count` : Count for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.count_percent` : Count Percent for particular columns.
        """
    ...
    def kurtosis(
        ...
        **agg_kwargs,
        ...
        """
        Calculates the kurtosis of the VastFrame to obtain a measure
        ...
        By aggregating the VastFrame with kurtosis, we can gain valuable
        insights into the data's distribution characteristics.

        .. warning::

            To compute kurtosis, vastorbit needs to execute multiple
            queries. It necessitates, at a minimum, a query that
            includes a subquery to perform this type of aggregation.
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_kurtosis_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_kurtosis_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.kurtosis` : Kurtosis for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.skewness` : Skewness for particular columns.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.std` : Standard Deviation for particular columns.
        """
    ...
    def mad(
        ...
        **agg_kwargs,
        ...
        .. warning::
            ...
            To compute mad, vastorbit needs to execute multiple
            queries. It necessitates, at a minimum, a query that
            includes a subquery to perform this type of aggregation.
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_mad_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_mad_table.html
        ...
        .. seealso::
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.std` : Mean Absolute Deviation for particular columns.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.mad` : Standard Deviation for a specific column.
        """
    ...
    def max(
        ...
        **agg_kwargs,
        ...
        """
        Aggregates the VastFrame by applying the ``MAX`` aggregation,
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_max_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_max_table.html
        ...
        .. seealso::
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.min` : Minimum for particular columns.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.max` : Maximum for a specific column.
        """
    ...
    def median(
        ...
        **agg_kwargs,
        ...
        """
        Aggregates the VastFrame using the ``MEDIAN`` or ``APPROX_MEDIAN``
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_median_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_median_table.html
        ...
        .. seealso::
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.max` : Maximum for particular columns.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.min` : Maximum for particular columns.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.mean` : Mean for a specific column.
        """
    ...
    def min(
        ...
        **agg_kwargs,
        ...
        """
        Aggregates the VastFrame by applying the ``MIN`` aggregation,
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_min_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_min_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.min` : Minimum for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.max` : Maximum for particular columns.
        """
    ...
    def product(
        ...
        **agg_kwargs,
        ...
        """
        Aggregates the VastFrame by applying the ``product``
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_product_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_product_table.html
        ...
        .. seealso::
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.aggregate` : Aggregates for particular columns.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.quantile` : Quantile Aggregates for particular columns.
        """
    ...
    def quantile(
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_quantile_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_quantile_table.html
        ...
        return self.aggregate(
            func=[vastorbit_agg_name(prefix + f"{float(item) * 100}%") for item in q],
            columns=columns,
            **agg_kwargs,
        )
    ...
    def sem(
        ...
        **agg_kwargs,
        ...
        technique to perform analysis and aggregation on the VastFrame.
        ...
        When we aggregate the VastFrame using ``sem``, we gain insights
        ...
        .. warning::
            ...
            To compute sem, vastorbit needs to execute multiple
            queries. It necessitates, at a minimum, a query that
            includes a subquery to perform this type of aggregation.
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_sem_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_sem_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.sem` : Standard Error of Mean for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.mad` : Mean Absolute Deviation for particular columns.
        """
    ...
    def skewness(
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_skewness_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_skewness_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.kurtosis` : Kurtosis for a specific column.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.skewness` : Skewness for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.std` : Standard Deviation for particular columns.
        """
    ...
    def std(
        ...
        **agg_kwargs,
        ...
        """
        Aggregates the VastFrame using ``STDDEV`` aggregation
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_std_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_std_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.kurtosis` : Kurtosis for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.skewness` : Skewness for particular columns.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.std` : Standard Deviation for a specific column.
        """
    ...
    def sum(
        ...
        **agg_kwargs,
        ...
        """
        Aggregates the VastFrame using ``SUM`` aggregation, which
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_sum_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_sum_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.sum` : Sum for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.max` : Maximum for particular columns.
        """
    ...
    def var(
        ...
        **agg_kwargs,
        ...
        """
        Aggregates the VastFrame using ``VAR`` aggregation
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_var_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_var_table.html
    ...
    def count_percent(
        ...
        **agg_kwargs,
        ...
        """
        Performs aggregation on the VastFrame using a list of
        ...
        ----------
        columns: SQLColumns, optional
            List of VastColumn names. If empty, all VastColumns
            are used.
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_count_percent_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_count_percent_table.html
    ...
    def nunique(
        ...
        **agg_kwargs,
        ...
        """
        When aggregating the VastFrame using `nunique` (cardinality),
        vastorbit employs the COUNT DISTINCT function to determine the
        number of unique values in a particular column. It also offers
        the option to use APPROX_DISTINCT, a more efficient
        ...
        .. hint::
            ...
            that using APPROX_DISTINCT can significantly
        ...
        **agg_kwargs
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_nunique_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_nunique_table.html
        ...
        .. seealso::
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.duplicated` : Duplicate Values for particular columns.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.nunique` : Cardinaility for a specific column.
        """
    ...
    def duplicated(
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_duplicated_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_duplicated_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.nunique` : Cardinality for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.nunique` : Cardinality for particular columns.
        """
        columns = format_type(columns, dtype=list)
        if len(columns) == 0:
            columns = self.get_columns()
        ...
        if count:
            total = _executeSQL(
                query=f"""
                    SELECT 
                        /*+LABEL('VastFrame.duplicated')*/ COUNT(*) 
                    FROM {main_table}""",
...
class vDCAgg(vDCEval):
    ...
    def aggregate(self, func: list) -> TableSample:
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDCAgg_aggregate_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDCAgg_aggregate_table.html
    ...
    def describe(
        ...
        """
        This function aggregates the VastColumn using multiple statistical
        aggregations such as minimum (min), maximum (max), median, cardinality
        (unique), and other relevant statistics. The specific aggregations
        applied depend on the data types of the VastColumn. For example,
        numeric columns are aggregated with numerical aggregations (min, median,
        max...), while categorical columns are aggregated using categorical ones
        (cardinality, mode...). This versatile function provides valuable insights
        into the dataset's statistical properties and can be customized to meet
        ...
        method: str, optional
            The describe method.
             ...
             - cat_stats:
                ...
                column for each VastColumn category.
        ...
        max_cardinality: int, optional
            ...
            VastColumn is considered as categorical.
        numcol: str, optional
            Numerical  VastColumn  to  use  when  the   parameter
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDCAgg_describe_num_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDCAgg_describe_num_table.html
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDCAgg_describe_cat_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDCAgg_describe_cat_table.html
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDCAgg_describe_stats_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDCAgg_describe_stats_table.html
        ...
        assert (method != "cat_stats") or (numcol), ValueError(
            "The parameter 'numcol' must be a VastFrame column if the method is 'cat_stats'"
        )
        ...
        if (is_date) and method != "categorical":
            ...
            cast_e = "AS INT)" if (self._parent[numcol].isbool()) else ""
                    ...
                    (SELECT 
                     ...
                     FROM 
                        (SELECT 
                         ...
                         GROUP BY {self} 
                         ORDER BY COUNT(*) DESC 
                         OFFSET {max_cardinality + 1}) VASTORBIT_SUBTABLE) 
                     ORDER BY count DESC"""
            query_result = _executeSQL(
                query=f"""
                    WITH vdf_table AS 
                        (SELECT 
                            /*+LABEL('VastColumn.describe')*/ * 
    ...
    def aad(self) -> PythonScalar:
        ...
        absolute deviation of data points from their mean, offering
        valuable insights into data variability and dispersion.
        When we aggregate the VastColumn using ``aad``, we gain an
        understanding of how data points deviate from the mean on
        average, which is particularly useful for assessing data
        spread and the magnitude of deviations.
        ...
        .. seealso::
            ...
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.aad` :
    ...
    def avg(self) -> PythonScalar:
        ...
        .. seealso::
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.avg` : Aggregations for particular columns.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.max` : Maximum for particular columns.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.min` : Minimum for particular columns.
        """
    ...
    def kurtosis(self) -> PythonScalar:
        """
        Calculates the kurtosis of the VastColumn to obtain a measure
        of the data's peakedness or tailness. The kurtosis statistic
        helps us understand the shape of the data distribution.
        It quantifies whether the data has heavy tails or is more peaked
        relative to a normal distribution.

        By aggregating the VastColumn with kurtosis, we can gain valuable
        insights into the data's distribution characteristics.
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.std` : Standard Deviation for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.kurtosis` : Kurtosis for particular columns.
        """
    ...
    def mad(self) -> PythonScalar:
        ...
        of data points around the median, and it is particularly
        valuable for assessing the robustness of data in the
        presence of outliers. When we aggregate the VastColumn
        using ``mad``, we gain insights into the variability and
        the degree to which data points deviate from the median.
    ...
    def median(
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.mean` : Mean for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.median` : Median for particular columns.
        """
    ...
    def product(self) -> PythonScalar:
        """
        Aggregates the VastColumn by applying the ``product``
        aggregation function. This function computes the
        product of values within the dataset, providing
        insights into the multiplication of data points.
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.aggregate` : Aggregations for a specific column.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.quantile` : Quantile Aggregates for a specific column.
        """
    ...
    def sem(self) -> PythonScalar:
        """
        Leverages the ``sem`` (Standard Error of the Mean) aggregation
        technique to perform analysis and aggregation on the VastColumn.
        Standard Error of the Mean is a valuable statistical measure used
        to estimate the precision of the sample mean as an approximation
        of the population mean.

        When we aggregate the VastColumn using ``sem``, we gain insights
        into the variability or uncertainty associated with the sample
        mean. This measure helps us assess the reliability of the sample
        mean as an estimate of the true population mean.
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.mad` : Mean Absolute Deviation for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.sem` : Standard Error of Mean for particular columns.
        """
    ...
    def skewness(self) -> PythonScalar:
        """
        Utilizes the ``skewness`` aggregation method to analyze and
        aggregate the VastColumn. Skewness, a measure of the asymmetry
        in the data's distribution, helps us understand the data's
        deviation from a perfectly symmetrical distribution. When we
        aggregate the VastFrame using skewness, we gain insights into
        the data's tendency to be skewed to the left or right, or if
    ...
    def sum(self) -> PythonScalar:
        ...
        .. seealso::
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.sum` : Sum for particular columns.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.max` : Maximum for a specific colum.
        """
    ...
    def mode(self, dropna: bool = False, n: int = 1) -> PythonScalar:
        ...
        in the VastColumn. It's a practical method for identifying the
        ...
        result = _executeSQL(
            f"""
            SELECT 
                /*+LABEL('VastColumn.mode')*/ {self} 
            FROM (
                SELECT 
                    {self}, 
                    COUNT(*) AS _vastorbit_cnt_ 
                FROM {self._parent}
                {where}GROUP BY {self} 
                ORDER BY _vastorbit_cnt_ DESC 
                LIMIT {n}) VASTORBIT_SUBTABLE 
                ORDER BY _vastorbit_cnt_ ASC 
                LIMIT 1""",
    ...
    def value_counts(self, k: int = 30) -> TableSample:
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_value_counts_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_value_counts_table.html
        ...
        .. seealso::
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.nunique` : Cardinality for a specific column.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.duplicated` : Duplicated values for particular columns.
        """
    ...
    def topk(self, k: int = -1, dropna: bool = True) -> TableSample:
        ...
        .. ipython:: python
            ...
            result = data["x"].topk()
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_topk_table.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_vDFAgg_topk_table.html
        ...
        result = _executeSQL(
            query=f"""
            SELECT 
                /*+LABEL('VastColumn.topk')*/
                {alias_sql_repr} AS {self},
                COUNT(*) AS _vastorbit_cnt_,
                100.00000000 * COUNT(*) / {self._parent.shape()[0]} AS percent
            FROM {self._parent}
            {where} 
    ...
    def distinct(self, **kwargs) -> list:
        ...
        within a VastColumn. It's a valuable method for exploring
        ...
        .. seealso::
            ...
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.aggregate` : Aggregates for particular columns.
        """
        alias_sql_repr = to_varchar(
            self.category(), self._alias, cast_in_all_cases=False
        )
        if "agg" not in kwargs:
            query = f"""
                SELECT 
                    /*+LABEL('VastColumn.distinct')*/ 
                    {alias_sql_repr} AS {self} 
                FROM {self._parent} 
                WHERE {self} IS NOT NULL 
                GROUP BY {self} 
            ...
            query = f"""
                SELECT 
                    /*+LABEL('VastColumn.distinct')*/ {self} 
                FROM 
                    (SELECT 
                        {alias_sql_repr} AS {self}, 
                        {kwargs['agg']} AS vastorbit_agg 
                     FROM {self._parent} 
                     WHERE {self} IS NOT NULL 
                     GROUP BY 1) x 
                ORDER BY vastorbit_agg DESC"""
        query_result = _executeSQL(
            query=query,
