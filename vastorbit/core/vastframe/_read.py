...
from vastorbit.core.vastframe._utils import vDFUtils, vDCUtils
...
class vDFRead(vDFUtils):
    ...
    def __getitem__(self, index: Any) -> Any:
        if isinstance(index, slice):
            assert index.step in (1, None), ValueError(
                "VastFrame doesn't allow slicing having steps different than 1."
            )
            ...
            return _executeSQL(
                query=f"""
                    SELECT /*+LABEL('VastFrame.__getitem__')*/ 
                        {', '.join(columns)} 
                    FROM {self}
                    {self._get_last_order_by()} 
            ...
            is_sql = False
            if hasattr(index, "object_type") and index.object_type == "VastColumn":
                index = index._alias
    ...
    def idisplay(self) -> None:
        ...
        interactive tables for all :py:class:`~VastFrame`.
        """
    ...
    def get_columns(self, exclude_columns: Optional[SQLColumns] = None) -> list[str]:
        ...
        exclude_columns: SQLColumns, optional
            List of the VastColumns names to
        ...
        .. ipython:: python
            ...
            result = vdf
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_read_get_columns_data.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_read_get_columns_data.html
        ...
        .. seealso::
            ...
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.head` :
                Get head of the :py:class:`~VastFrame`.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.tail` :
                Get tail of the :py:class:`~VastFrame`.
        """
    ...
    def head(self, limit: int = 5) -> TableSample:
        ...
        .. ipython:: python
            ...
            result = data.head()
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_read_head.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_read_head.html

        .. seealso::

            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.head` :
                Get head of the :py:class:`~VastColumn`.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.tail` :
                Get tail of the :py:class:`~VastFrame`.
        """
    ...
    def iloc(
        ...
        """
        Returns a part of the :py:class:`~VastFrame`
        ...
        columns: SQLColumns, optional
            ...
            :py:class:`~VastColumn` to include in the
            result.  If empty, all :py:class:`~VastColumn`
            are selected.
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_read_iloc.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_read_iloc.html
        ...
        .. seealso::
            ...
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.select` :
                Select columns from the :py:class:`~VastFrame`.
        """
        ...
        for column in columns:
            cast = to_varchar(self[column].category(), column, cast_in_all_cases=False)
        ...
        try:
            ...
            if self._vars["has_dpnames"]:
                ...
                raise
        pre_comp = self._get_catalog_value("vastorbit_COUNT")
    ...
    def shape(self) -> tuple[int, int]:
        ...
        .. ipython:: python
            ...
            result = vdf
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_read_shape.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_read_shape.html

        We can get the shape of the
        :py:class:`~VastFrame` by:
        ...
        .. seealso::
            ...
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.iloc` :
                Select rows from the :py:class:`~VastFrame`.
        """
    ...
    def tail(self, limit: int = 5) -> TableSample:
        """
        Returns the tail of the :py:class:`~VastFrame`.
        ...
        .. ipython:: python
            ...
            result = data.tail()
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_read_tail.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_read_tail.html

        .. note::

            This function appends a OFFSET statement at
            the end of the query generated by vastorbit.
        ...
        .. seealso::
            ...
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.head` :
                Get head of the :py:class:`~VastFrame`.
            | ``VastColumn.``:py:meth:`~vastorbit.VastColumn.tail` :
                Get tail of the :py:class:`~VastColumn`.
        """
        return self.iloc(limit=limit, offset=-1)

    @save_vastorbit_logs
    def select(self, columns: SQLColumns) -> "VastFrame":
        """
        Returns a copy of the :py:class:`~VastFrame`
        with only the selected :py:class:`~VastColumn`.

        Parameters
        ----------
        columns: SQLColumns
            List of the :py:class:`~VastColumn` to
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_read_select.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_read_select.html
        ...
        .. seealso::
            ...
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.iloc` :
                Get custom rows from a :py:class:`~VastFrame`.
        """
...
class vDCRead(vDCUtils):
    def __getitem__(self, index) -> Any:
        if isinstance(index, slice):
            assert index.step in (1, None), ValueError(
                "VastColumn doesn't allow slicing having steps different than 1."
            )
            index_stop = index.stop
            index_start = index.start
            if not isinstance(index_start, int):
            ...
            if self.isarray():
                ...
                query = f"""
                    ...
                    FROM {self._parent}) VASTORBIT_SUBTABLE"""
    ...
    def head(self, limit: int = 5) -> TableSample:
        """
        Returns the head of the :py:class:`~VastColumn`.
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_read_vDCRead_head.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_read_vDCRead_head.html
    ...
    def iloc(self, limit: int = 5, offset: int = 0) -> TableSample:
        """
        Returns a part of the :py:class:`~VastColumn`
        (delimited by an ``offset`` and a ``limit``).
    ...
    def nlargest(self, n: int = 10) -> TableSample:
        """
        Returns the ``n`` largest :py:class:`~VastColumn`
        elements.
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_read_nlargest.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_read_nlargest.html

        .. note::

            This function can be employed to explore the dataset,
            and the output is a :py:class:`~vastorbit.core.tablesample.base.TableSample`—an in-memory
        ...
        .. seealso::
            ...
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.select` :
                Select columns from the :py:class:`~VastFrame`.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.nsmallest` :
                Select the smallest values of a column from the :py:class:`~VastFrame`.
        """
        query = f"""
            SELECT 
                * 
    ...
    def nsmallest(self, n: int = 10) -> TableSample:
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_read_nsmallest.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_read_nsmallest.html
        ...
        .. seealso::
            ...
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.select` :
                Select columns from the :py:class:`~VastFrame`.
            | ``VastFrame.``:py:meth:`~vastorbit.VastFrame.nlargest` :
                Select the largest values of a column from the :py:class:`~VastFrame`.
        """
        return TableSample.read_sql(
            f"""
            SELECT 
    ...
    def tail(self, limit: int = 5) -> TableSample:
        """
        Returns the tail of the :py:class:`~VastColumn`.
        ...
        .. ipython:: python
            ...
            html_file = open("SPHINX_DIRECTORY/figures/core_VastFrame_read_vDCRead_tail.html", "w")
            html_file.write(result._repr_html_())
            html_file.close()

        .. raw:: html
            :file: SPHINX_DIRECTORY/figures/core_VastFrame_read_vDCRead_tail.html
