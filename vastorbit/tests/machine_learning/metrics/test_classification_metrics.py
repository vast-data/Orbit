...
import vastorbit.machine_learning.metrics.classification as vo_metrics
import numpy as np
import pandas as pd
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "y",
            "log_loss",
        ),  # failed all - VAST uses log base 10, sklean uses natural log(e)
...
class TestClassificationMetrics:
    ...
    def test_master_classification_metrics(
        ...
        if metric_name in ["best_cutoff"] and compute_method != "binary":
            ...
            pytest.skip("VAST binning issue needs to be fixed.")
        elif metric_name in ["log_loss"]:
            pytest.skip("VAST has default base 10, sklean uses natural log (e)")

        # vastorbit logic
        def get_VAST_metrics():
            """
            getter function for VAST metrics
            """
            if metric_name in ["confusion_matrix"]:
                _vo_res = getattr(vo_metrics, metric_name)(
                ...
                if compute_method == "binary":
                    ...
                    _vo_res = getattr(vo_metrics, metric_name)(
                        ...
                        average=compute_method,
                        labels=(
                            [str(label_num) for label_num in labels_num]
                            if metric_name
                            in [
                                "roc_auc_score",
                                "prc_auc_score",
                ...
                vo_report_pdf = vo_report.to_pandas()
                vo_report_pdf = vo_report_pdf.drop("avg_micro", axis=1)
                _vo_res = vo_report_pdf.to_dict()
            else:
                _vo_res = getattr(vo_metrics, metric_name)(
        ...
        print(f"{metric_name} - VAST: {vo_res}, sklearn: {skl_res}")

        if isinstance(vo_res, dict):
            for v, s in zip(vo_res.values(), skl_res.values()):
                assert v == pytest.approx(
                    s, rel=rel_tolerance, abs=abs_tolerance
                ), f"VAST: {vo_res}, sklearn: {skl_res}"
        else:
