...
class TestBaseTreeModel:
    ...
    def test_get_tree(self, get_vo_model, model_class):
        """
        test function - test_get_tree
        """
        model = get_vo_model(model_class).model
        if model_class in [
            "RandomForestRegressor",
            "RandomForestClassifier",
            "XGBRegressor",
    ...
    def test_plot_tree(self, get_vo_model, model_class):
        """
    ...
    def test_to_graphviz(self, get_vo_model, model_class):
        """
        test function - test_to_graphviz
        """
        gvz_tree_0 = get_vo_model(model_class).model.to_graphviz(
            tree_id=0,
...
class TestRegressionTreeModel:
    ...
    def test_get_score(self, model_class, get_vo_model, key_name, expected):
        """
        test function - test_get_score
        """
        score = get_vo_model(model_class).model.get_score()
    ...
    def test_score(
        ...
        self.abs_error_report_reg_tree[(model_class, py_metric_name)] = {
            ...
            "abs_pct_diff": ((vo_score - py_score) / (py_score if py_score else 1e-15))
            * 100,
        }
...
class TestClassificationTreeModel:
    ...
    def test_classification_report(
        ...
        report = getattr(vo_model_obj.model, fun_name)()
        vo_report_map = dict(zip(report["index"], report["value"]))

        py_report_map = classification_metrics(model_class)

        assert vo_report_map[metric] == pytest.approx(
            py_report_map[metric], rel=rel_abs_tol_map[model_class][metric]["rel"]
        )

    def test_confusion_matrix(self, model_class, get_vo_model, get_py_model):
        """
        test function - test_confusion_matrix
        """
        vo_res = get_vo_model(model_class).model.confusion_matrix()
    ...
    def test_cutoff_curve(self, model_class, get_vo_model, get_py_model):
        """
        test function - test_cutoff_curve
        """
        cutoff_curve = get_vo_model(model_class).model.cutoff_curve(show=False)
        _fpr, _tpr = cutoff_curve["false_positive"], cutoff_curve["true_positive"]
        vo_res = skl_metrics.auc(_fpr, _tpr)
    ...
    def test_lift_chart(self, model_class, get_vo_model, lift_chart_type, expected):
        """
        test function - test_lift_chart
        """
        lift_chart = get_vo_model(model_class).model.lift_chart(show=False)
    ...
    def test_prc_curve(self, model_class, get_vo_model, get_py_model):
        """
        test function - test_prc_curve
        """
        vo_prc_curve = get_vo_model(model_class).model.prc_curve(show=False)
        vo_recall, vo_precision = vo_prc_curve["recall"], vo_prc_curve["precision"]
        vo_res = skl_metrics.auc(vo_recall, vo_precision)
    ...
    def test_predict_proba(self, model_class, get_vo_model, get_py_model):
        """
        ...
        assert vo_res[:5] == pytest.approx(
            py_res[:5], rel=rel_abs_tol_map[model_class]["predict_proba"]["rel"]
        )

    def test_roc_curve(self, model_class, get_vo_model, get_py_model):
        """
        test function - test_roc_curve
        """
        vo_roc_curve = get_vo_model(model_class).model.roc_curve(show=False)
        vo_fpr, vo_tpr = (
            vo_roc_curve["false_positive"],
            vo_roc_curve["true_positive"],
        )
        vo_res = skl_metrics.auc(vo_fpr, vo_tpr)
...
class TestXGBModel:
    ...
    def test_to_json(self, model_class, get_vo_model, get_py_model):
        """
        ...
        if model_class == "XGBClassifier":
            ...
            vo_model_obj = get_vo_model(model_class, X=X, y=y)
        else:
            return_proba = False
            pred_method = "predict"
            vo_model_obj = get_vo_model(model_class)

        vo_model_obj.model.to_json(path=file_path)

        py_model_obj = get_py_model(model_class)
        py_model_obj.model.load_model(file_path)
        y_test_VAST = vo_model_obj.model.to_python(return_proba=return_proba)(
            py_model_obj.X
        )
        y_test_python = getattr(py_model_obj.model, pred_method)(py_model_obj.X)
        result = (y_test_VAST - y_test_python) ** 2
