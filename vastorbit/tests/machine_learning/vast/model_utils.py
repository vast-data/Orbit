...
import vastorbit.machine_learning.vast as vo_linear_model
import vastorbit.machine_learning.vast.cluster as vo_cluster
import vastorbit.machine_learning.vast.ensemble as vo_ensemble
import vastorbit.machine_learning.vast.svm as vo_svm
import vastorbit.machine_learning.vast.tree as vo_tree
import vastorbit.machine_learning.vast.tsa as vo_tsa
...
def get_model_attributes(model_class):
    ...
    attributes_map = {
        ...
        **dict.fromkeys(
            ["AR"], ["phi_", "intercept_", "feature_importances_", "mse_", "n_"]
        ),
        **dict.fromkeys(["MA"], ["theta_", "mu_", "mean_", "mse_", "n_"]),
        **dict.fromkeys(
            ["ARMA"], ["phi_", "theta_", "mean_", "feature_importances_", "mse_", "n_"]
        ),
        **dict.fromkeys(
            ["ARIMA"], ["phi_", "theta_", "mean_", "feature_importances_", "mse_", "n_"]
        ),
        **dict.fromkeys(
            ["KMeans"],
            [
    ...
    return attributes_map.get(
        model_class, ["coef_", "intercept_", "feature_importances_"]
    )


def get_train_function(model_class):
    """
    getter function to get VAST train function name
    """
    train_func_map = {
        **dict.fromkeys(
            ["RandomForestRegressor", "DecisionTreeRegressor", "DummyTreeRegressor"],
...
def get_predict_function(model_class):
    """
    getter function to get VAST predict function name
    """
    pred_func_map = {
        **dict.fromkeys(
            ["RandomForestRegressor", "DecisionTreeRegressor", "DummyTreeRegressor"],
...
def get_function_name(model_class):
    ...
    function_name_map = {
        **dict.fromkeys(
            ...
            {
                "vpy": ["vo_tree_classifier", "vo_linear", "vo_linear"],
        ...
        **dict.fromkeys(
            TIMESERIES_MODELS,
            {
                "vpy": ["vo_tree_regressor", "vo_timeseries", "vo_timeseries"],
        ...
        **dict.fromkeys(
            CLUSTER_MODELS,
            {
                "vpy": ["vo_tree_classifier", "vo_cluster", "vo_cluster"],
...
class LinearRegressionInitializer:
    ...
    def __init__(self, datasetup_instance, **kwargs):
        ...
        self.model_name = f"vo_model_{self.datasetup_instance.model_class}"

    def vpy(self):
        """
        Model initializer function for VAST Linear Regression model
        """
        model = getattr(vo_linear_model, self.datasetup_instance.model_class)(
        ...
        print(f"vastorbit Training Parameters: {model.get_params()}")
...
class RidgeInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST Ridge model
        """
        model = getattr(vo_linear_model, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
...
class LassoInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST Lasso model
        """
        model = getattr(vo_linear_model, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
...
class ElasticNetInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST ElasticNet model
        """
        model = getattr(vo_linear_model, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
...
class LinearSVRInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST LinearSVR model
        """
        model = getattr(vo_svm, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
            tol=self.tol,
            C=self.C,
...
class PoissonRegressorInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST PoissonRegressor model
        """
        model = getattr(vo_linear_model, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
...
class RandomForestInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST RandomForest model
        """
        model = getattr(vo_tree, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
...
class DecisionTreeInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST DecisionTree model
        """
        model = getattr(vo_tree, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
...
class DummyTreeInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST DummyTree model
        """
        model = getattr(vo_tree, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
...
class XGBInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST XGBoost model
        """
        model = getattr(vo_ensemble, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
...
class ARInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST Autoregressor model
        """
        model = getattr(vo_tsa, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
            p=self.p,
            method=self.method,
...
class MAInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST MovingAverage model
        """
        model = getattr(vo_tsa, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
...
class ARMAInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST AutoRegressor MovingAverage model (d=0) model
        """
        model = getattr(vo_tsa, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
...
class KMeansInitializer:
    ...
    def vpy(self):
        """
        Model initializer function for VAST KMeans model
        """
        model = getattr(vo_cluster, self.datasetup_instance.model_class)(
            name=f"{self.datasetup_instance.schema_name}.{self.model_name}",
            overwrite_model=self.overwrite_model,
...
class DataSetUp:
    ...
    def vo_tree_regressor(self):
        """
        Data setup function for VAST tree regression model(s)
        """
    ...
    def vo_tree_classifier(self):
        """
        Data setup function for VAST tree classification model(s)
        """
...
class TrainModel:
    ...
    def vo_tree_regressor(self):
        """
        function to train VAST regression model(s)
        """
    ...
    def vo_tree_classifier(self):
        """
        function to train VAST tree classification model(s)
        """
        _X = [f'"{i}"' for i in self.datasetup_instance.X]
        predictor_columns = ",".join(self.datasetup_instance.X)
    ...
    def vo_linear(self):
        """
        function to train VAST linear model(s)
        """
        self.model.fit(
    ...
    def vo_timeseries(self):
        """
        function to train VAST timeseries model(s)
        """
        self.model.fit(
            f"{self.datasetup_instance.schema_name}.{self.datasetup_instance.dataset_name}",
    ...
    def vo_cluster(self):
        """
        function to train VAST clustering model(s)
        """
        self.model.fit(
            f"{self.datasetup_instance.schema_name}.{self.datasetup_instance.dataset_name}",
            self.datasetup_instance.X,
...
class PredictModel:
    ...
    def vo_tree_regressor(self):
        """
        function to make predictions using VAST tree regression model(s)
        """
    ...
    def vo_tree_classifier(self):
        """
        function to make predictions using VAST tree classification model(s)
        """
        pred_vdf = self.model.predict(
            f"{self.datasetup_instance.schema_name}.{self.datasetup_instance.dataset_name}",
            name=f"{self.datasetup_instance.y[0]}_pred",
    ...
    def vo_linear(self):
        """
        function to make predictions using VAST linear model(s)
        """
        pred_vdf = self.model.predict(
            f"{self.datasetup_instance.schema_name}.{self.datasetup_instance.dataset_name}",
            name=f"{self.datasetup_instance.y[0]}_pred",
    ...
    def vo_timeseries(self):
        """
        function to make predictions using VAST timeseries model(s)
        """
        row_cnt = (
            current_cursor()
            .execute(
    ...
    def vo_cluster(self):
        """
        function to make predictions using VAST clustering model(s)
        """
        pred_vdf = self.model.predict(
            f"{self.datasetup_instance.schema_name}.{self.datasetup_instance.dataset_name}",
