...
class TestMath:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("VastFrame_column", ["age", "fare", "pclass", "survived"]),
            ("VastColumn", "age"),
    ...
    def test_abs(self, titanic_vd_fun, input_type, columns):
        ...
        if input_type == "VastFrame":
            titanic_vdf.abs()
            vo_res = titanic_vdf[columns].sum()
            py_res = titanic_pdf.abs()[columns].sum()
        elif input_type == "VastFrame_column":
            if isinstance(columns, list) and len(columns) > 1:
                _vo_res = titanic_vdf.abs(columns=columns)[columns].sum()
                vo_res = dict(zip(_vo_res["index"], _vo_res["sum"]))['"age"']
                py_res = dict(titanic_pdf[columns].abs().sum())["age"]
            else:
                vo_res = titanic_vdf.abs(columns=columns)[columns].sum()
                py_res = titanic_pdf[columns].abs().sum()
        else:
            if isinstance(columns, list) and len(columns) > 1:
                _vo_res = titanic_vdf[columns].abs()[columns].sum()
                vo_res = dict(zip(_vo_res["index"], _vo_res["sum"]))['"age"']
                py_res = dict(titanic_pdf[columns].abs().sum())["age"]
            else:
                vo_res = titanic_vdf[columns].abs()[columns].sum()
                py_res = titanic_pdf[columns].abs().sum()
    ...
    def test_binary_operator(self, titanic_vd_fun, func, columns, scalar):
        ...
        vo_res = getattr(titanic_vd_fun[columns], func)(scalar)[columns].sum()
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            #        "name": "REGEXP_EXTRACT({}, '([A-Za-z])+\.')",
            #    },
            #    None,
            # ),
            (["age"], "VastColumn", "POWER({}, 2)", None),
            (["age"], "VastColumn", "POWER({}, 2)", "age_pow2"),
        ],
    )
    ...
    def test_apply_fun(self, titanic_vd_fun, data, columns, vo_func, py_func):
        """
        ...
        vo_data = sample_data if data == "sample_data" else titanic_vd_fun
        ...
        py_data[columns] = (
            ...
            if vo_data[columns].isnum() and not vo_data[columns].isarray()
            else py_data[columns]
        )

        vo_data[columns].apply_fun(func=vo_func)
        vo_res = vo_data[columns].sum()
    ...
    def test_date_part(self, smart_meters_vd, part, columns):
        ...
        vo_res = smart_meters_copy[columns].date_part(part)[columns].sum()
    ...
    @pytest.mark.parametrize(
        "col_type, data, column, expected_lengths",
        [
            (
                "string",
                {
                ...
                "name",
                [5, 3, 7, 5],  # Lengths: Alice=5, Bob=3, Charlie=7, David=5
            ),
            (
                "array",
                {
                ...
                "tags",
                [2, 1, 3, 0],  # Array cardinalities
            ),
        ],
    )
    def test_get_len(self, col_type, data, column, expected_lengths):
        """
        Test get_len() for string and array columns
        """
        # Create VastFrame
        vdf = VastFrame(data)

        # Get lengths using VastFrame
        vo_result = vdf[column].get_len()
        vo_lengths = vo_result._parent.to_list()
        vo_lengths_flat = [item[0] for item in vo_lengths]  # Flatten list of tuples
        vo_sum = sum(vo_lengths_flat)

        # Expected sum
        expected_sum = sum(expected_lengths)

        # Verify individual lengths
        expected_lengths.sort()
        vo_lengths_flat.sort()
        assert (
            vo_lengths_flat == expected_lengths
        ), f"Expected lengths {expected_lengths}, got {vo_lengths_flat}"

        # Verify sum
        assert vo_sum == expected_sum, f"Expected sum {expected_sum}, got {vo_sum}"

        print(f"Column type: {col_type}")
        print(f"vastorbit lengths: {vo_lengths_flat}")
        print(f"Expected lengths: {expected_lengths}")
        print(f"vastorbit sum: {vo_sum}")
    ...
    def test_round(self, titanic_vd_fun, column, n):
        ...
        vo_res = titanic_vd_fun[column].round(n)[column].sum()

        py_res = titanic_pdf[column].round(n).sum()

        print(f"vastorbit Result: {vo_res} \nPython Result :{py_res}\n")

        assert vo_res == pytest.approx(py_res, rel=1e-04)
    ...
    def test_slice(self, smart_meters_vd, length, unit, start, column, expected):
        ...
        assert vo_res.replace(tzinfo=None) == pytest.approx(expected)
    ...
    def test_analytic(
        ...
        if func in ["aad", "count"]:
            ...
            if func == "first_value":
                vo_res = titanic_vd_fun[name].max()
                py_res = titanic_pdf[columns].iloc[0]
            elif func == "last_value":
                # vo_res = titanic_vd_fun.analytic(func=func, columns=columns, by=by, order_by=order_by, name=name)[by].isin("Belfast, NI")[name]
                vo_res = titanic_vd_fun[by].isin("Belfast, NI")[name]
                py_res = titanic_pdf.groupby(by).last(columns).loc["Belfast, NI"]
            elif func == "ema":
                vo_res = titanic_vd_fun[:10][name].sum()
                py_res = (
                    titanic_pdf["age"]
            ...
            vo_res = titanic_vd_fun[name].sum()
    ...
    @pytest.mark.parametrize(
        "column, func", [("sex", "CASE {} WHEN NULL THEN 0 ELSE 1 END")]
    )
    def test_applymap(self, titanic_vd_fun, column, func):
        ...
        """
        titanic_pdf = titanic_vd_fun.to_pandas()
        vo_res = titanic_vd_fun.applymap(func=func, numeric_only=False)[column].sum()
