...
# Stats
from vastorbit.sql.functions.conditional import case_when
from vastorbit.sql.functions.math import E, INF, PI, NAN, TAU
from vastorbit.sql.functions.math import (
    isinf,
    isnan,
)
from vastorbit.sql.functions.null_handling import zeroifnull
...
class TestRolling:
    ...
    @pytest.mark.parametrize(
        "vo_func, py_func, window, columns, by, order_by",
        [
            ...
            # ),  # few rows values are not matching with vastorbit. comparing sample records
            (
                "count",
                "count",
                (-1, 1),
            ...
            #    None,
            #    {"name": "asc", "ticket": "desc"},
            # ),  # all rows value for corr are not matching with vastorbit. comparing sample records
            # (
            #    "cov",
            #    "cov",
            #    (-1, 1),
            ...
            #    None,
            #    {"name": "asc", "ticket": "desc"},
            # ),  # all rows value for cov are not matching with vastorbit. comparing sample records
            # (
            #    "kurtosis",
            #    "kurt",
            #    (-2, 1),
    ...
    def test_rolling(
        ...
        """
        test function - str_extract for VastColumns
        """
        ...
        titanic_vd_fun = (
            titanic_vd_fun[:50]
            if vo_func in ["corr", "cov", "beta", "skewness", "kurtosis", "jb"]
            else titanic_vd_fun
        )
        ...
        print(
            f"Rolling Function : {vo_func} \nvastorbit Result: {vo_res} \nPython Result :{py_res}\n"
        )

        assert vo_res == pytest.approx(py_res, rel=1e-02)
