...
class TestText:
    ...
    def test_regexp(
        ...
        """
        test function - regexp for VastFrame
        """
        titanic_pdf = titanic_vd_fun.to_pandas()

        _vo_res = titanic_vd_fun.regexp(
            column=column,
            pattern=pattern,
            method=method,
            position=position,
    ...
    def test_str_contains(self, titanic_vd_fun, column, pat):
        """
        test function - str_contains for VastColumns
        """
        titanic_pdf = titanic_vd_fun.to_pandas()

        _vo_res = titanic_vd_fun[column].str_contains(pat=pat)[column][:5].to_list()
        vo_res = list(chain(*_vo_res))
    ...
    def test_str_count(self, titanic_vd_fun, column, pat):
        """
        test function - str_count for VastColumns
        """
        titanic_pdf = titanic_vd_fun.to_pandas()

        _vo_res = titanic_vd_fun[column].str_count(pat=pat)[column][:5].to_list()
        vo_res = list(chain(*_vo_res))
    ...
    def test_str_extract(self, titanic_vd_fun, column, pat):
        ...
        _vo_res = titanic_vd_fun[column].str_extract(pat=pat)[column][:5].to_list()
        vo_res = list(chain(*_vo_res))
    ...
    @pytest.mark.parametrize(
        "column, to_replace, value", [("name", r" ([A-Za-z])+\.", "vastorbit")]
    )
    def test_str_replace(self, titanic_vd_fun, column, to_replace, value):
        """
        test function - str_replace for VastColumns
        """
        titanic_pdf = titanic_vd_fun.to_pandas()
    ...
    def test_str_slice(self, titanic_vd_fun, column, start, end):
        """
        test function - str_slice for VastColumns
        """
        titanic_pdf = titanic_vd_fun.to_pandas()
