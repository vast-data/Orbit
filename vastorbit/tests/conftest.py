...
def load_test_schema():
    ...
    vastorbit.create_schema(schema_name)
    yield schema_name
    vastorbit.drop(schema_name, method="schema")


@pytest.fixture(scope="session")
def dummy_vd():
    """
    Create a dummy VastFrame
    """
    ...
    dummy = vastorbit.VastFrame(list(zip(arr1, arr2)), ["check 1", "check 2"])
    yield dummy
...
def dummy_scatter_vd():
    """
    Create a dummy VastFrame that is suitable for scatter plot
    """
...
def dummy_date_vd():
    """
    Create a dummy VastFrame that has date data
    """
    data_size = DUMMY_TEST_SIZE
    median = 500
    qrtr_1 = 200
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        dummy = vastorbit.read_pandas(dummy, temp_path=temp_dir)
    yield dummy
...
def dummy_probability_data():
    """
    Create a dummy VastFrame that has probability data
    """
    ...
    dummy = vastorbit.VastFrame({"y_true": prob, "y_score": pred})
    yield dummy
...
def dummy_dist_vd():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        dummy = vastorbit.read_pandas(data_all, temp_path=temp_dir)
...
def dummy_line_data_vd():
    """
    Create a dummy VastFrame that has time data
    """
    ...
    yield vastorbit.VastFrame({"date": years, "values": values, "category": category})


@pytest.fixture(scope="session")
def dummy_pred_data_vd():
    """
    Create a dummy VastFrame that has prediction as a column
    """
    data_size = DUMMY_TEST_SIZE
    x_val = np.linspace(0, 10, data_size)
    ...
    yield vastorbit.VastFrame({"X": x_val, "Y": y_val, "Z": z_val, "Category": pred})


@pytest.fixture(scope="module")
def titanic_vd(schema_loader):
    """
    Create a dummy VastFrame for titanic dataset
    """
...
def iris_vd(schema_loader):
    """
    Create a dummy VastFrame for iris dataset
    """
...
def amazon_vd(schema_loader):
    """
    Create a dummy VastFrame for amazon dataset
    """
...
def pred_cl_dataset_multilevel():
    ...
    vdf = VastFrame(input_relation, usecols=["y_t", "y_s"])
    yield vdf, y_true, y_pred, labels


@pytest.fixture(scope="module")
def winequality_vpy(schema_loader):
    with winequality_vo_main(schema_loader) as result:
        yield result


@pytest.fixture(scope="function")
def winequality_vo_fun(schema_loader):
    with winequality_vo_main(schema_loader) as result:
        yield result
...
# @pytest.fixture(scope="function")
@contextmanager
def winequality_vo_main(schema_loader):
    """
    Create a dummy VastFrame for winequality data
    """
...
def market_vd(schema_loader):
    """
    Create a dummy VastFrame for market dataset
    """
...
def smart_meters_vd(schema_loader):
    """
    Create a dummy VastFrame for smart_meters dataset
    """
...
def laliga_vd(schema_loader):
    """
    Create a dummy VastFrame for laliga dataset
    """
...
def airline_vd(schema_loader):
    """
    Create a dummy VastFrame for airline_passengers dataset
    """
...
def pop_growth_vd(schema_loader):
    """
    Create a dummy VastFrame for load_pop_growth dataset
    """
...
def gapminder_vd(schema_loader):
    """
    Create a dummy VastFrame for load_gapminder dataset
    """
...
def world_vd(schema_loader):
    """
    Create a dummy VastFrame for load_world dataset
    """
...
def cities_vd(schema_loader):
    """
    Create a dummy VastFrame for load_cities dataset
    """
