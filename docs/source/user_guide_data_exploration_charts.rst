...
.. ipython:: python
  ...
  from vastorbit.datasets import load_titanic, load_iris, load_world, load_amazon, load_africa_education
...
When dealing with numerical data types, the process is different. VAST needs to discretize the numerical features to draw them. You can choose the bar width (``h`` parameter) or let vastorbit compute an optimal width using the Freedman-Diaconis rule.
...
vastorbit uses the same process for other graphics, like 2-dimensional histograms and bar charts.
...
.. note:: vastorbit has three main plotting libraries. Look at :ref:`chart_gallery` section for all the different plots.
...
Scatter and bubble plots are also useful for identifying patterns in your data. Note, however, that these methods don't use aggregations; vastorbit downsamples the data before plotting. You can use the ``max_nb_points`` to limit the number of points and avoid unnecessary memory usage.
...
For more information on scatter look at :py:func:`~vastorbit.VastFrame.scatter`
...
Time-series plots are also available with the :py:func:`~vastorbit.VastFrame.plot` method.
