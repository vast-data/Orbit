...
The model is good. Let's add it in our :py:mod:`~vastorbit.VastFrame`.
...
Since :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans` clustering is sensitive to unnormalized data, let's normalize our new predictors.
...
Let's add the clusters in the :py:mod:`~vastorbit.VastFrame`.
