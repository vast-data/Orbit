...
We can examine the missing values with the :py:func:`~vastorbit.VastFrame.count` method.
...
The missing values for ``boat`` are MNAR; missing values simply indicate that the passengers didn't pay for a lifeboat. We can replace all the missing values with a new category ``No Lifeboat`` using the :py:func:`~vastorbit.VastFrame.fillna` method.
...
The features ``embarked`` and ``fare`` have a couple missing values. Instead of using a technique to impute them, we can just drop them with the :py:func:`~vastorbit.VastFrame.dropna` method.
...
The :py:func:`~vastorbit.VastFrame.fillna` method offers many options. Let's use the :py:func:`help` function to view its parameters.
