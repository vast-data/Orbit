...
A vector autoregression (:py:mod:`~vastorbit.machine_learning.vast.tsa.arima.VAR`) model can be very good to do the predictions. But first, let's encode the states to look at their influence.
...
The :py:mod:`~vastorbit.VastFrame` memorizes all of our operations on the data to dynamically generate the correct SQL statement and passes computation and aggregation to VAST.
...
We can see clearly a high correlation for some variables. We can use them to compute a ``SARIMAX`` model, but we'll stick to a :py:mod:`~vastorbit.machine_learning.vast.tsa.arima.VAR` model for this study.

Let's compute the total number of deaths and cases to create our :py:mod:`~vastorbit.machine_learning.vast.tsa.arima.VAR` model.
...
Let's create a :py:mod:`~vastorbit.machine_learning.vast.tsa.arima.VAR` model to predict the number of COVID-19 deaths and cases in the USA.
