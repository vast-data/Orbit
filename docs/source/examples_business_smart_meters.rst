...
Create the :py:mod:`~vastorbit.VastFrame` of the datasets:
...
.. note:: You can let VAST automatically decide the data type, or you can manually force the data type on any column as seen above.
...
In vastorbit, you can interpolate joins; VAST will find the closest timestamp to the key and join the result.
...
The dataset ``sm_meters`` is pretty important. In particular, the type of residence is probably a good predictor for electricity usage. We can create clusters of the different regions with :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans` clustering based on longitude and latitude. Let's find the most suitable ``k`` using an elbow curve and scatter plot.
...
Based on the scatter plot, five seems like the optimal number of clusters. Let's verify this hypothesis using an :py:func:`~vastorbit.machine_learning.model_selection.elbow` curve.
...
The elbow curve seems to confirm that five is the optimal number of clusters, so let's create a :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans` model with that in mind.
...
Let's add our clusters to the :py:mod:`~vastorbit.VastFrame`.
...
.. ipython:: python
    ...
    vo.drop("sm_consumption_weather_region", method = "table")
    sm_consumption_weather_region.to_db(
        "sm_consumption_weather_region",
        relation_type = "table",
    )
    sm_consumption_weather_region_clean = vo.VastFrame("sm_consumption_weather_region")
...
.. ipython:: python
    ...
    from vastorbit.machine_learning.model_selection.statistical_tests import het_breuschpagan
