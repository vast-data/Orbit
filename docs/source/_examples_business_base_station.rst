...
..
    ...
    You can download the Jupyter notebook of this study `here <https://github.com/vastdata-dev/vastorbit/blob/master/examples/business/base_station/base_station.ipynb>`_.
    ...
    .. code-block:: python
        ...
        # Creating the schema
        vo.drop("shanghai", method = "schema")
        vo.create_schema("shanghai")
        ...
        # Shanghai Districts #
        ######################
        shanghai_districts = vo.read_csv(
    ...
    predefined dataset in vastorbit, and then plot on a map of China 
    ...
    Let's examine the network activity of each of our districts. To do this, we need vastorbit's Geospatial, which leverages VAST geospatial functions. We begin by creating an index for the districts and then find the intersection between connections and districts. We'll visualize this with a bar chart race, which reflects each district's cumulative activity duration through time.

    .. code-block:: python

        from vastorbit.sql.geo import create_index, intersect
    ...
    We create virtual base stations by grouping existing base stations in 100 clusters. Clustering is performed using :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans` clustering on Euclidean coordinates of the base stations. Each cluster represents a wider coverage of connections.
    ...
    .. ipython:: python
        ...
        model = KMeans("shanghai.kmeans_bs", n_clusters = 100)
    ...
    .. ipython:: python
        ...
        vo.drop("shanghai.bs", method = "table");
    ...
    .. ipython:: python
        ...
        bs_workload_90 = bs_workload.groupby(
            ["latitude", "longitude"],
            "APPROX_PERCENTILE(workload USING PARAMETERS percentile=0.90) AS workload",
        );
        ...
        vo.drop("shanghai.bs_workload_90", method = "table")
    ...
    The :py:func:`~vastorbit.machine_learning.model_selection.elbow` curve seems to indicate that 4 would be a good number of clusters, so let's try k = 4 and view the weighted :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans` algorithm's suggested positions for new base stations based on the centers of the clusters.
    ...
    .. ipython:: python
        ...
        model = KMeans("shanghai.new_bs_kmeans", n_clusters = 4);
        model.fit(bs_xy, ["longitude", "latitude"])
    ...
    With the predictive power of AutoML, we can predict the workload of the base stations. :py:mod:`~vastorbit.machine_learning.vast.automl.AutoML` is a powerful technique that tests multiple models to maximize the input score.
    ...
    .. ipython:: python
        ...
        vo.drop("shanghai.bs_metrics", method = "table")
        bs_metrics = bs.join(
            cworkload_bs,
            how = "left",
            on = {"cluster": "cluster"},
