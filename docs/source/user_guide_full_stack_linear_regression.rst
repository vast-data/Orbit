...
We'll begin by repeating the data preparation process of the previous section and export the resulting :py:mod:`~vastorbit.VastFrame` to VAST.
...
Let's create our principal component analysis (:py:mod:`~vastorbit.machine_learning.vast.decomposition.PCA`) model.
...
We can verify the Gauss-Markov assumptions with our :py:mod:`~vastorbit.machine_learning.vast.decomposition.PCA` model.
