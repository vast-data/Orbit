...
Let's build the first :py:mod:`~vastorbit.machine_learning.vast.svm.LinearSVC` to predict if a flower is an Iris setosa.
...
We can add this probability to the :py:mod:`~vastorbit.VastFrame`.
...
We have another excellent model. Let's add it to the :py:mod:`~vastorbit.VastFrame`.
...
Let's evaluate our final model (the combination of two :py:mod:`~vastorbit.machine_learning.vast.svm.LinearSVC`).
