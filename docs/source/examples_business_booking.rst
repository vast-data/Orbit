...
We start by using the :py:func:`~vastorbit.VastFrame.sessionize` method to create the variable ``session_id``. We can then use this variable to aggregate the data.
...
Let's create our :py:mod:`~vastorbit.machine_learning.vast.linear_model.LogisticRegression` model.
...
Let's add our prediction to the :py:mod:`~vastorbit.VastFrame`.
