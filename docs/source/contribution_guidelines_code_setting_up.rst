...
.. code-block:: bash
    ...
    git clone git@github.com:YOURUSERNAME/vastorbit.git
    cd vastorbit

Your GitHub repository **YOURUSERNAME/vastorbit** will be called "origin" in Git. You should also setup **VAST/vastorbit** as an "upstream" remote.

.. code-block:: bash

    git remote add upstream git@github.com:VAST/vastorbit.git
    git fetch upstream
...
**Override with a configuration file** called `vastorbit/tests/vp_test.conf`. This is a file that would be ignored by git. We created an example `vastorbit/tests/vp_test.conf.example` for your reference.
...
.. code-block:: bash
    ...
    tox -- vastorbit/tests/VastFrame/test_vDF_combine_join_sort.py

    # Run all tests on the python version 3.6:
    tox -e py36 -- vastorbit/tests

    # Run all tests on the python version 3.7 with verbose result outputs:
    tox -e py37 -v -- vastorbit/tests
    ...
    tox -e py37,py38 -- vastorbit/tests/VastFrame/test_vDF_combine_join_sort.py::TestvDFCombineJoinSort::test_vDF_append
...
When upstream (VAST/vastorbit) has changed, you should rebase your work. The **rebase** command creates a linear history by moving your local commits onto the tip of the upstream commits.
...
When you think your work is ready to be pulled into *vastorbit*, you should create a pull request(PR) at GitHub.
