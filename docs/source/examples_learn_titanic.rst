...
.. ipython:: python
    ...
    model = TfidfVectorizer()
    model.fit(titanic, "ticket", x="Name")
    res = model.transform(titanic, "ticket", x="Name")["word"].topk()
...
.. ipython:: python
    ...
    model = TfidfVectorizer()
    model.fit(titanic, "ticket", x="cabin")
    res = model.transform(titanic, "ticket", x="cabin")["word"].topk()
...
Let's move on to outliers. We have several tools for locating outliers (:py:mod:`~vastorbit.machine_learning.vast.LocalOutlierFactor`, :py:mod:`~vastorbit.machine_learning.vast.cluster.DBSCAN`, :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans`...), but we'll just use winsorization in this example. Again, "fare" has many outliers, so we'll start there.
