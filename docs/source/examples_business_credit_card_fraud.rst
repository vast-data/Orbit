...
``Time`` and ``Amount`` are the only features that have not been transformed with :py:mod:`~vastorbit.machine_learning.vast.decomposition.PCA`.

- **V1, V2,..., V28:** principal components from :py:mod:`~vastorbit.machine_learning.vast.decomposition.PCA`.
...
:py:mod:`~vastorbit.machine_learning.vast.linear_model.LogisticRegression` works well with monotonic relationships. Since we have a lot of independent features that correlate with the response, it should be a good first model to use.
...
.. ipython:: python
    ...
    fig.write_html("SPHINX_DIRECTORY/figures/examples_creditcardfraud_feature_importances_1.html")

.. raw:: html
    :file: SPHINX_DIRECTORY/figures/examples_creditcardfraud_feature_importances_1.html

Some :py:mod:`~vastorbit.machine_learning.vast.decomposition.PCA` components seem to be very relevant and will be essential for finding anomalies.
...
- **Machine Learning:** We need to use easily-deployable algorithms to perform real-time fraud detection. Isolation forests and :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans` can be easily deployed and they work well for detecting anomalies.
- **Rules & Thresholds:** The z-score can be an efficient solution for detecting global outliers.
- **Decomposition:** Robust :py:mod:`~vastorbit.machine_learning.vast.decomposition.PCA` is another technique for detecting outliers.
...
We begin by examining :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans` clustering, which partitions the data into k clusters.

We can use an elbow curve to find a suitable number of clusters. We can then add more clusters then the amount suggested by the :py:func:`~vastorbit.machine_learning.model_selection.elbow` curve to create clusters mainly composed of anomalies. Clusters with relatively fewer elements can then be investigated by an expert to label the anomalies.
...
Notice that clusters with fewer elemenets tend to contain much more fraudulent events than the others. This methodology makes :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans` a good algorithm for catching collective outliers. Combining :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans` with other techniques like ``Z-score``, we can find most of the outliers of the distribution.
...
We can catch outliers with a neighbors score. Again, the main problem with these sorts of algorithms is that what they have in precision, they lack in speed, which makes them unsuitable for scoring new data. This is why it's important to focus on scalable techniques like :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans`.

Other Techniques
+++++++++++++++++

Other scalable techniques that can solve this problem are robust :py:mod:`~vastorbit.machine_learning.vast.decomposition.PCA` and isolation forest.
