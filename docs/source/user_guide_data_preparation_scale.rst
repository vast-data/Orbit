...
Normalizing data is crucial when using machine learning algorithms because of how sensitive most of them are to un-scaled data. For example, the neighbors-based and :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans` algorithms use the ``p-distance`` in their learning phase.
...
To demonstrate data scaling in vastorbit, we will use the well-known ``titanic`` dataset.
...
These lie in different numerical intervals so it's probably a good idea to scale them. To scale data in vastorbit, we can use the :py:func:`~vastorbit.VastFrame.scale` method.
