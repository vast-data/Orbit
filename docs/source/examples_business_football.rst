...
To compute a :py:mod:`~vastorbit.machine_learning.vast.cluster.KMeans` model, we need to find a value for ``k``. Let's draw an :py:func:`~vastorbit.machine_learning.model_selection.elbow` curve to find a suitable number of clusters.
...
Let's add the prediction to the :py:mod:`~vastorbit.VastFrame`.
...
Let's add the predictions to the :py:mod:`~vastorbit.VastFrame`.
