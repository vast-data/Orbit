...
This document will help you create the documentation for entire vastorbit library automatically using Sphinx.

## Before you start

- You need to clone the entire vastorbit repo
- Open vastorbit/_config/config.py and change the theme to sphinx (this ensures that the created plots/graphs are transparent which is needed to accessibility)
...
```
- Open a bash terminal, cd into ``vastorbit/docs`` and install all requirements
...
```
replacement_word = "/project/data/vastorbit/docs"
```
...
- Uninstall vastorbit if it is already installed
```
...
- Install the updated vastorbit with corrected directory paths
```
pip install ../.
```
